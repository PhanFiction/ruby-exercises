def blank_seating_chart(number_of_rows, seats_per_row)
  ...
  # change with it
  newArr = Array.new(number_of_rows) { Array.new(seats_per_row) }
  newArr
end
...
def find_favorite(array_of_hash_objects)
 ...
 # alternative using .find   array_of_hash_objects.find { |data| data[:is_my_favorite?] == true }
  array_of_hash_objects.each do |data|
    return data if data[:is_my_favorite?] == true
